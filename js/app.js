...
// PLAN B UYARISI GÖSTERME FONKSİYONU
function showOfflineWarning() {
    const warningBox = document.createElement('div');
    warningBox.className = 'alert alert-warning fixed-bottom m-3 shadow-lg border-warning';
    warningBox.style.zIndex = '9999';
    warningBox.innerHTML = `
        <div class="d-flex align-items-center justify-content-between">
            <span><strong> DİKKAT:</strong> Canlı API erişilemiyor, örnek veri (sample.json) gösteriliyor.</span>
...
async function loadLaunches(type) {
    ...
    try {
        ...
        if (!response.ok) throw new Error("API Hatası"); // Hata fırlat ki catch yakalasın
        const data = await response.json();

        renderLaunches(data, type, container); // Veriyi ekrana bas 

    } catch (error) {
        console.warn(" API Çöktü! Plan B (Yedek Veri) devreye giriyor...");
        

        try {
            const fallbackResponse = await fetch('data/sample.json'); // Yerel dosyayı oku
            const fallbackData = await fallbackResponse.json();
            

            renderLaunches(fallbackData, type, container);
            

            showOfflineWarning(); 

        } catch (fileError) {
            container.innerHTML = `<div class="col-12 alert alert-danger">Ne API ne de yedek dosya çalıştı.</div>`;
        }
    }
}

// --- KODUNU BURADAN AL ---
function renderLaunches(data, type, container) {
    let launches = [];
    
    // Veriyi Kesme (Son 3 veya İlk 3)
    if (type === 'latest') {
        // Veri API'den geliyorsa ters çevir, sample.json ise olduğu gibi al
        launches = data.reverse ? data.reverse().slice(0, 3) : data.slice(0, 3);
    } else {
    ...
    if (launches.length === 0) {
        container.innerHTML = '<div class="col-12 text-white-50 text-center">Veri yok.</div>';
        return;
    }
    ...
    launches.forEach(launch => {
        ...
        if (launch.date_utc) {
            dateStr = new Date(launch.date_utc).toLocaleDateString('tr-TR', { day: 'numeric', month: 'long', year: 'numeric' });
        }
